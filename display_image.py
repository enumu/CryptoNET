...
def train_CyptoGAN_latent(Alice, Bob , FFN , train_data_loader , valid_data_loader , batch_size , c_optim , n_epoch):
  min_train_loss = 100
  for j in range(n_epoch):
    ...
    if (train_loss / len(train_data_loader)) < min_train_loss : 
      ...
      torch.save(FFN.state_dict(),"FFN_2.pth")
...
if __name__ == "__main__":
    ...
    ConvCoder.load_state_dict(torch.load(args.chkpt_file))
    train_loader , valid_loader = get_train_valid_loader(args.dataset_path,args.batch_size,True,1234)
    Show_Convolutional_Encoder_Image(train_loader,ConvCoder)
